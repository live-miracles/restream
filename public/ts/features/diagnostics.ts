...
function colorizeJson(str: string): string {
  return escapeHtml(str).replace(
    ...
    (match, strVal, colon, boolNull, num) => {
      if (strVal && colon)
        return `<span class="text-info">${strVal}</span>${colon}`;
...
function formatDiagnosticOutput(result: DiagnosticResult): string {
  ...
  if (result.name === "Ring Buffer Health")
    return formatRingBufferOutput(trimmed);
  if (result.name === "Active Outputs")
    return formatActiveOutputsOutput(trimmed);
...
function cleanDiagnosticValue(value: string): string {
  ...
  return trimmed.replace(/\bNone\b/g, "--").replace(/Some\(([^()]*)\)/g, "$1");
}
...
function formatRingBufferOutput(str: string): string {
  ...
  for (const rawLine of str.split(/\r?\n/)) {
    ...
    const reader = line.match(
      /^-\s*(.+):\s*lag=(\d+)\s+slots,\s*overflows=(\d+),\s*packet_age=(.+)ms$/,
    );
    if (inReaders && reader) {
    ...
    notes.push(
      line.replace(
        "Buffer is empty — no media has been received recently.",
        "Buffer is empty — active readers are caught up with the producer.",
      ),
    );
  }
...
function formatActiveOutputsOutput(str: string): string {
  ...
  return `<div class="space-y-3">${blocks
    .map((block) => {
      const protocol = cleanDiagnosticValue(
        block.fields.protocol || "--",
      ).toUpperCase();
...
function renderList(): void {
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (result)
      html += `<span class="${statusClass} font-bold">${statusIcon}</span>`;
