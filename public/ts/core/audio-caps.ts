...
// GET /api/v1/audio-caps. This module fetches it once on dashboard init and caches it
...
export function detectAudioPlatform(url: string): AudioPlatform {
  ...
  if (/(^|\.)vdocipher\.com$/.test(host) || /(^|\.)vd0\.co$/.test(host))
    return "vdocipher";
  return "generic";
}

// Infer protocol from the output URL scheme. http/https → hls.
export function detectAudioProtocol(
  url: string,
  fallback: AudioProtocol = "rtmp",
): AudioProtocol {
  let scheme = "";
  try {
