import { readRelayConfig } from '../utils/relay-config';
...
function extractStreamResource(streamId: string): string | null {
    // MediaMTX streamid format: "publish:live/<streamKey>"
    const publishMatch = /^publish:(.+)$/.exec(streamId);
    if (publishMatch?.[1]) return publishMatch[1];
    // SRS streamid format: "#!::r=<path>,m=publish"
    const srsMatch = /(?:^|[?,&#]|::|,)r=([^,&#]+)/.exec(streamId);
    if (srsMatch?.[1]) return srsMatch[1].replace(/^\/+/, '');
    return null;
}
