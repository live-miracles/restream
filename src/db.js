// SQLite data layer. The only place in the app that reads or writes the database.
// Exposes stable query and mutation helpers for pipelines, outputs, stream keys, and job history
// so the rest of the codebase never builds SQL ad hoc. Schema migrations run at open time.
const fs = require('fs');
...
// ── Schema (inlined from db/schema.js) ───────────────

function ensureStreamKeysTable(db) {
    db.prepare(
        `
...
function ensurePipelinesTable(db) {
    db.prepare(
        `
...
function ensurePipelineColumns(db) {
...
function ensureOutputsTable(db) {
...
function ensureOutputColumns(db) {
...
function ensureOutputIndexes(db) {
...
function dedupeJobsByPipelineOutput(db) {
...
function ensureJobLogsTable(db) {
    db.prepare(
        `
...
function ensureJobLogColumns(db) {
...
function migrateLegacyJobLogsForeignKey(db) {
...
function ensureJobLogIndexes(db) {
...
function setupDatabaseSchema(db) {
    ...
    ensureStreamKeysTable(db);
    ensurePipelinesTable(db);
    ensurePipelineColumns(db);
    ensureOutputsTable(db);
    ensureOutputColumns(db);
    ensureOutputIndexes(db);
    ensureJobsTable(db);
    dedupeJobsByPipelineOutput(db);
    ensureJobIndexes(db);
    ensureJobLogsTable(db);
    ensureJobLogColumns(db);
    migrateLegacyJobLogsForeignKey(db);
    ensureJobLogIndexes(db);
    ensureMetaTable(db);
}
...
// Database invariants used throughout the app:
// - outputs.desired_state stores operator intent independently from jobs.status
// - jobs keeps at most one current row per (pipeline_id, output_id)
// - pipeline-scoped history entries use output_id = null and share the same log table
// - outputs always belong to a pipeline and jobs always belong to an output+pipeline pair
...
function listJobLogsByPipelineFiltered(
    pipelineId,
    { since = null, until = null, limit = null, order = 'desc', prefixes = [] } = {},
) {
    const clauses = ['pipeline_id = ?', 'output_id IS NULL'];
...
// History APIs expect parsed JSON payloads, so normalize DB rows once here instead of repeating
// JSON.parse guards in every caller.
...
// Group helpers by table domain so callers can treat this file as the only persistence boundary.
module.exports = {
    /* stream key helpers */
    ...
    listJobLogsByPipelineFiltered(pipelineId, filters = {}) {
        return parseLogRows(listJobLogsByPipelineFiltered(pipelineId, filters));
    },
