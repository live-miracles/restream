...
class StreamKey {
  ...
  static validate(obj = {}) {
    if (obj.key && typeof obj.key !== 'string') return { ok: false, error: 'key must be a string' };
    return { ok: true };
  }
...
class Pipeline {
  ...
  static validate(obj = {}) {
    if (!obj.name || typeof obj.name !== 'string') return { ok: false, error: 'name is required' };
    return { ok: true };
  }
...
// javascript
class Job {
  constructor({ id, outputId, pid = null, status = 'pending', startedAt = null, stoppedAt = null, exitCode = null } = {}) {
    this.id = id || Date.now().toString();
