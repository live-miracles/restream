...
cc -O2 -I"$PREFIX/include" "$ROOT/test/srt-bond-server.c" \
    "$PREFIX/lib/libsrt.a" -lstdc++ -lssl -lcrypto -ldl -lpthread -lm \
    -o "$PREFIX/bin/restream-srt-bond-server"
cc -O2 -I"$PREFIX/include" "$ROOT/test/srt-bond-client.c" \
    "$PREFIX/lib/libsrt.a" -lstdc++ -lssl -lcrypto -ldl -lpthread -lm \
    -o "$PREFIX/bin/restream-srt-bond-client"
...
PKG_CONFIG_PATH="$PREFIX/lib/pkgconfig" cc -O2 -static \
    ...
    -o "$PREFIX/bin/restream-ffmpeg-capabilities"
